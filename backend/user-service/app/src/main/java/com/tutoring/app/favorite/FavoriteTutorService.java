...
public class FavoriteTutorService {
  ...
  private FavoriteTutorDTO toDto(FavoriteTutor favorite) {
    ...
    return FavoriteTutorDTO.builder().id(favorite.getId()).studentId(favorite.getStudent().getId())
            .tutorId(tutor.getId()).tutorUsername(tutor.getUsername())
            .tutorPhotoPath(tutor.getPhotoPath()).tutorDescription(tutor.getDescription()).build();
  }
}
