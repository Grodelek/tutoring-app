...
public class TutorOfferController {
    ...
    @PostMapping("/send")
    public OfferResponseDTO makeOffer(@RequestBody TutorOfferDTO offerDTO) throws Exception {
        return tutorOfferService.makeOffer(offerDTO);
    }

    @PostMapping("/accept/{offerId}")
    public OfferResponseDTO acceptOffer(@PathVariable UUID offerId){
        return tutorOfferService.acceptOffer(offerId);
    }

    @PostMapping("/decline/{offerId}")
    public OfferResponseDTO declineOffer(@PathVariable UUID offerId){
        return tutorOfferService.declineOffer(offerId);
    }

    @PostMapping("/confirm-payment/{offerId}")
    public OfferResponseDTO confirmPayment(@PathVariable UUID offerId){
        return tutorOfferService.confirmPayment(offerId);
    }

    @GetMapping("/my")
    public List<OfferResponseDTO> getMyBookings(){
        return tutorOfferService.getMyStudentBookings();
    }
}
